...
// Cipher 加解密接口：明文字符串进出，密文为大写 HEX 字符串。
type Cipher interface {
	Encrypt(text string) string
	Decrypt(hex string) string
}

// NewCipher 按算法 ID（GUID）创建对应加密器；未知 GUID 返回错误。
func NewCipher(algoID string) (Cipher, error) {
	...
	default:
		// 第三代 9 + 第二代 PC 6 注册项在 cipher_new.go（旧 9 项保持原样，回归红线）
		return newCipherExt(algoID)
	}
}
